...
import { useHorizon, useWebAuth } from "../../hooks/stellar"
...
export function useWithdrawalState(account: Account) {
  ...
  const handleWithdrawalFormSubmission = async (
    ...
    try {
      ...
      const webauthMetadata = await WebAuth.fetchWebAuthData(horizon, asset.getIssuer())
  ...
  const requestWithdrawal = async (withdrawalRequest: WithdrawalRequestData, authToken?: string) => {
    try {
      ...
      if (initResponse.type === "kyc") {
        ...
        const transactionID = initResponse.data.id || (initResponse.data as any).identifier
